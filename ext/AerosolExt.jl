...
function EarthSciMLBase.couple2(
    ...
    @constants(
        ...
        FC = 0.15,
        [unit = u"1",
            description = "Cloud fraction (prescribed climatological mean; FC=1.0 = 100% cloud everywhere over-depleted H2O2 ~4x in the CONUS A/B. INTERIM until coupled from GEOSFP A3cld CLOUD)"],
        L_cld = 0.3,
...
function EarthSciMLBase.get_odefunction(
    ...
    function run(du, u, p, t)
        ...
        Threads.@threads :static for j in 1:ncell
            ...
            #     regime where the activity/water fixed point fails to converge (36% of
            #     cell-solves in the first CONUS hour burned all 40 outer iterations for
            #     a bounded |du| <= k_mt*1e-3 ppb/s ≈ 3e-6 ppb/s answer of ~0).
