...
function EarthSciMLBase.couple2(c::GasChem.GEOSChemGasPhaseCoupler, e::EarthSciData.NEI2016MonthlyEmisCoupler)
    c, e = c.sys, e.sys

    @constants(
        MW_ACET = 58.09*10^-3, [unit = u"kg/mol", description="Acetone molar mass"],
        MW_ALD2 = 44.06*10^-3, [unit = u"kg/mol", description="Acetaldehyde molar mass"],
        MW_BENZ = 78.12*10^-3, [unit = u"kg/mol", description="Benzene molar mass"],
    ...
    # nmol_emissions / mol_air = (kg_emissions / MW_emission * nmolpermol) / (m3_air / R / T * P)
    @constants(
        P_conv=1/6.02214076e+23, [unit = us"mol/cm^2", description="Number density conversion (handles lack of units in GEOS-Chem pressure)"],
        mw_air = 28.97e-3, [unit = u"kg/mol", description="Molar mass of air"],
        g = 9.80665, [unit = u"m/s^2", description="Acceleration due to gravity"],
    )
...
function EarthSciMLBase.couple2(c::GasChem.GEOSChemGasPhaseCoupler, gfp::EarthSciData.GEOSFPCoupler)
    c, gfp = c.sys, gfp.sys
    ...
    ConnectorSystem([
            c.T ~ gfp.I3₊T,
            c.num_density * P_conv * mw_air * g ~ gfp.P,
        ], c, gfp)
end
