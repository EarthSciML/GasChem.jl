...
@testitem "MethaneOxidation: Structural Verification" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    ch4_inputs = [
        :CH4, :CH3, :CH3O2, :CH3O, :CH3OOH, :HCHO, :HCO,
...
@testitem "MethaneOxidation: Table 6.1 Rate Constants" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    params = parameters(sys)
    param_dict = Dict(
        Symbol(p) => ModelingToolkit.getdefault(p)
            for p in params if ModelingToolkit.hasdefault(p)
    )

    # Bimolecular rate constants (m^3/s)
    @test param_dict[:k1] ≈ 2.45e-12 * exp(-1775 / 298) * 1.0e-6
    @test param_dict[:k3] ≈ 7.7e-12 * 1.0e-6
    @test param_dict[:k4] ≈ 5.2e-12 * 1.0e-6
    @test param_dict[:k5] ≈ 3.5e-13 * 1.0e-6
    @test param_dict[:k6] ≈ 1.9e-15 * 1.0e-6
    @test param_dict[:k7] ≈ 3.6e-12 * exp(200 / 298) * 1.0e-6
    @test param_dict[:k8] ≈ 1.9e-12 * 1.0e-6
    @test param_dict[:k10] ≈ 9.0e-12 * 1.0e-6
    @test param_dict[:k13] ≈ 5.2e-12 * 1.0e-6
    @test param_dict[:k15] ≈ 8.1e-12 * 1.0e-6

    # Termolecular rate constants (m^6/s)
    @test param_dict[:k2_0] ≈ 1.0e-30 * 1.0e-12
    @test param_dict[:k14_0] ≈ 5.7e-32 * 1.0e-12
    @test param_dict[:k17_0] ≈ 6.0e-34 * 1.0e-12

    # Photolysis rates (s^-1)
    @test param_dict[:j9] ≈ 5.0e-6
    @test param_dict[:j11] ≈ 3.0e-5
    @test param_dict[:j12] ≈ 4.0e-5
    @test param_dict[:j16] ≈ 8.0e-3
end
...
@testitem "MethaneOxidationODE: Rate Constants and Parameters" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test param_dict[:k_CO_OH] ≈ 2.4e-13 * 1.0e-6
    @test param_dict[:k_OH_NO2] ≈ 1.0e-11 * 1.0e-6
    @test param_dict[:k_HO2_HO2] ≈ 2.9e-12 * 1.0e-6
    @test param_dict[:k_NO_O3] ≈ 1.9e-14 * 1.0e-6

    # External OH source (m^-3/s)
    @test param_dict[:P_OH_ext] ≈ 1.0e6 * 1.0e6
end
...
@testitem "MethaneOxidation: Reaction Rate R1 (CH4+OH)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    input_vals = Dict(
        compiled.CH4 => 4.5e19, compiled.OH => 1.0e12, compiled.CH3 => 1.0e10,
        compiled.CH3O2 => 1.0e14, compiled.CH3O => 1.0e10, compiled.CH3OOH => 1.0e15,
        compiled.HCHO => 1.0e16, compiled.HCO => 1.0e10, compiled.HO2 => 1.0e14,
        compiled.H => 1.0e8, compiled.NO => 2.5e15, compiled.NO2 => 2.5e16,
        compiled.O => 1.0e8, compiled.O2 => 5.25e24, compiled.M => 2.5e25
    )
    ...
    expected_R1 = k1 * 4.5e19 * 1.0e12
    @test sol[compiled.R1] ≈ expected_R1 rtol = 1.0e-6
...
@testitem "MethaneOxidation: Reaction Rate R3 (CH3O2+NO)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    # R3 = k3 * CH3O2 * NO
    k3 = 7.7e-12 * 1.0e-6
    expected_R3 = k3 * 1.0e14 * 2.5e15
    @test sol[compiled.R3] ≈ expected_R3 rtol = 1.0e-6
...
@testitem "MethaneOxidation: HCHO Production (P_HCHO = R6 + R8)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    # P_HCHO = R6 + R8
    @test sol[compiled.P_HCHO] ≈ sol[compiled.R6] + sol[compiled.R8] rtol = 1.0e-6

    # Check individual rates against expected values
    k6 = 1.9e-15 * 1.0e-6
    k8 = 1.9e-12 * 1.0e-6
    expected_R6 = k6 * 1.0e10 * 5.25e24
    expected_R8 = k8 * 1.0e15 * 1.0e12
    @test sol[compiled.R6] ≈ expected_R6 rtol = 1.0e-6
    @test sol[compiled.R8] ≈ expected_R8 rtol = 1.0e-6
...
@testitem "MethaneOxidation: CH4 Loss Equals R1" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test sol[compiled.L_CH4] ≈ sol[compiled.R1] rtol = 1.0e-6
...
@testitem "MethaneOxidation: Net O3 Production (P_O3_gross = R15 + R3)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test sol[compiled.P_O3_gross] ≈ sol[compiled.R15] + sol[compiled.R3] rtol = 1.0e-6
...
@testitem "MethaneOxidation: Low NOx Regime" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    input_vals = Dict(
        compiled.CH4 => 4.5e19, compiled.OH => 1.0e12, compiled.CH3 => 1.0e10,
        compiled.CH3O2 => 1.0e14, compiled.CH3O => 1.0e10, compiled.CH3OOH => 1.0e15,
        compiled.HCHO => 1.0e16, compiled.HCO => 1.0e10, compiled.HO2 => 2.0e14,
        compiled.H => 1.0e8, compiled.NO => 1.0e13, compiled.NO2 => 1.0e14,
        compiled.O => 1.0e8, compiled.O2 => 5.25e24, compiled.M => 2.5e25
    )
...
@testitem "MethaneOxidation: All Reaction Rates Positive" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    for R_sym in [
            compiled.R1, compiled.R2, compiled.R3, compiled.R4, compiled.R5,
...
@testitem "MethaneOxidation: R1 Scales Linearly with OH" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    prob_b = remake(prob_a, p = [compiled.OH => 2.0e12])
    sol_b = solve(prob_b)

    # R1 = k1 * CH4 * OH, so doubling OH should double R1
    @test sol_b[compiled.R1] / sol_a[compiled.R1] ≈ 2.0 rtol = 1.0e-6
end
...
@testitem "MethaneOxidationODE: Numerical Integration" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    u0 = [
        ...
        compiled_sys.CH3O2 => 1.0e14,
        compiled_sys.CH3O => 1.0e10,
        compiled_sys.CH3OOH => 1.0e15,
        compiled_sys.HCHO => 1.0e16,
        compiled_sys.HCO => 1.0e10,
        compiled_sys.CO => 2.5e18,
        compiled_sys.H2 => 1.3e19,
        compiled_sys.OH => 1.0e12,
        compiled_sys.HO2 => 1.0e14,
        compiled_sys.H => 0.0,
        compiled_sys.NO => 2.5e15,
        compiled_sys.NO2 => 2.5e16,
        compiled_sys.O => 0.0,
        compiled_sys.O3 => 1.0e18,
    ...
    for v in unknowns(compiled_sys)
        vals = sol[v]
        @test all(x -> x >= -1.0e-6 * max(abs(vals[1]), 1.0), vals)
    end
end
...
@testitem "MethaneOxidationODE: Approximate NOx Conservation" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test NOx_total_final ≈ NOx_total_initial rtol = 1.0e-4
end
...
@testitem "MethaneOxidationODE: CH4 Decreases Monotonically" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    tspan = (0.0, 100.0)
    prob = ODEProblem(compiled_sys, u0, tspan)
    sol = solve(prob, Rosenbrock23(), abstol = 1.0e-8, reltol = 1.0e-8, saveat = 1.0)

    @test string(sol.retcode) == "Success"

    ch4_vals = sol[compiled_sys.CH4]
    for i in 2:length(ch4_vals)
        @test ch4_vals[i] <= ch4_vals[i - 1] + 1.0e-3 * ch4_vals[1]
    end
