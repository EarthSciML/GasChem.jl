...
@testitem "COOxidation: Eq 6.18 HO2 Steady-State (High NOx)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    prob = NonlinearProblem(
        compiled,
        Dict(
            compiled.CO => CO_val, compiled.OH => OH_val, compiled.HO2 => HO2_val,
    ...
    @test HO2_ss_val > 1.0e12
    @test HO2_ss_val < 1.0e16
...
@testitem "COOxidation: Rate Constants at 298 K" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test param_dict[:k_HO2_NO] ≈ 8.1e-12 * 1.0e-6
    @test param_dict[:k_HO2_HO2] ≈ 2.9e-12 * 1.0e-6
    @test param_dict[:k_OH_NO2] ≈ 1.0e-11 * 1.0e-6
    @test param_dict[:k_HO2_O3] ≈ 2.0e-15 * 1.0e-6
    @test param_dict[:k_OH_O3] ≈ 7.3e-14 * 1.0e-6
end
...
@testitem "OzoneProductionEfficiency: Rate Constants" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test param_dict[:k_HO2_NO] ≈ 8.1e-12 * 1.0e-6
    @test param_dict[:k_RO2_NO] ≈ 8.0e-12 * 1.0e-6
    @test param_dict[:k_OH_NO2] ≈ 1.0e-11 * 1.0e-6
end
...
@testitem "OzoneProductionEfficiency: OPE Calculation" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    prob = NonlinearProblem(
        compiled,
        Dict(
            compiled.OH => OH_val, compiled.HO2 => HO2_val, compiled.RO2 => RO2_val,
...
@testitem "OzoneProductionEfficiency: High NOx - Low OPE" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    prob_low = NonlinearProblem(
        ...
        Dict(
            compiled.OH => OH_val, compiled.HO2 => HO2_val, compiled.RO2 => RO2_val,
            compiled.NO => 2.5e14, compiled.NO2 => 5.0e14
        );
        build_initializeprob = false
    )
    sol_low = solve(prob_low)
    ...
    prob_high = remake(prob_low, p = [compiled.NO => 2.5e16, compiled.NO2 => 5.0e16])
    sol_high = solve(prob_high)
    ...
    @test sol_low[compiled.OPE] ≈ sol_high[compiled.OPE] rtol = 1.0e-6
...
@testitem "COOxidation: Qualitative - Positive Production and Loss" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    for OH_val in [1.0e11, 1.0e12, 1.0e13]
        for HO2_val in [1.0e13, 1.0e14, 1.0e15]
            for NO_val in [1.0e14, 1.0e16, 1.0e18]
                NO2_val = 2 * NO_val
