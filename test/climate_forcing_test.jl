...
@testitem "ClimateSensitivity structural verification" setup=[ClimateSetup] tags=[:climate] begin
    ...
    @test "ΔT_s(t)" in var_names
    @test "ΔT_0(t)" in var_names
    @test "λ_0(t)" in var_names
    @test "ΔF_e(t)" in var_names
    @test "ΔT_unrealized(t)" in var_names
    @test "ΔF_r(t)" in var_names
end

@testitem "ClimateSensitivity equation verification" setup=[ClimateSetup] tags=[:climate] begin
    ...
    prob = ODEProblem(csys, [], (0.0, 1.0), [csys.λ => 0.8, csys.ΔF => 3.7])
    sol = solve(prob, saveat=1.0)

    ΔTs_computed = sol[csys.ΔT_s][1]
    @test isapprox(ΔTs_computed, 0.8 * 3.7, rtol=1e-6)
end

@testitem "ClimateSensitivity no-feedback response" setup=[ClimateSetup] tags=[:climate] begin
    ...
    prob = ODEProblem(csys, [], (0.0, 1.0), [csys.ΔF => 3.7])
    sol = solve(prob, saveat=1.0)

    λ0_computed = sol[csys.λ_0][1]
    ΔT0_computed = sol[csys.ΔT_0][1]

    # λ₀ should be approximately 1.25/3.7 = 0.338
    @test isapprox(λ0_computed, 1.25 / 3.7, rtol=1e-6)

    # ΔT₀ should be λ₀ × ΔF
    @test isapprox(ΔT0_computed, λ0_computed * 3.7, rtol=1e-6)
end

@testitem "ClimateSensitivity feedback factor" setup=[ClimateSetup] tags=[:climate] begin
...
@testitem "ClimateSensitivity unrealized warming" setup=[ClimateSetup] tags=[:climate] begin
    # Test Eq. 23.7: Unrealized warming calculation
    ...
    prob = ODEProblem(csys, [], (0.0, 1.0), [csys.λ => λ_val, csys.ΔF => ΔF_val, csys.ΔT_r => ΔTr_val])
    sol = solve(prob, saveat=1.0)

    ΔT_unrealized = sol[csys.ΔT_unrealized][1]

    # From Eq. 23.7: ΔT_unrealized = (ΔF - ΔF_r) × λ
    # where ΔF_r = ΔT_r / λ = 0.7 / 0.7 = 1.0 W/m²
    # So ΔT_unrealized = (1.7 - 1.0) × 0.7 = 0.49 K
    expected_unrealized = (ΔF_val - ΔTr_val / λ_val) * λ_val
    @test isapprox(ΔT_unrealized, expected_unrealized, rtol=1e-6)
...
@testitem "GHGForcing structural verification" setup=[ClimateSetup] tags=[:climate] begin
    ...
    vars = unknowns(sys)
    var_names = [string(Symbolics.tosymbol(v, escape=false)) for v in vars]
    @test "ΔF_total(t)" in var_names
    @test "ΔF_CO2(t)" in var_names
    @test "ΔF_CH4(t)" in var_names
end
...
@testitem "GHGForcing reference values" setup=[ClimateSetup] tags=[:climate] begin
    ...
    prob = ODEProblem(csys, [], (0.0, 1.0))
    sol = solve(prob, saveat=1.0)

    # Individual forcings from p. 1039
    @test isapprox(sol[csys.ΔF_CO2][1], 1.46, rtol=1e-6)
    @test isapprox(sol[csys.ΔF_CH4][1], 0.48, rtol=1e-6)
    @test isapprox(sol[csys.ΔF_N2O][1], 0.15, rtol=1e-6)
    @test isapprox(sol[csys.ΔF_O3][1], 0.4, rtol=1e-6)

    # Total forcing (sum of well-mixed GHGs + O3)
    total = 1.46 + 0.48 + 0.15 + 0.4 + 0.34
    @test isapprox(sol[csys.ΔF_total][1], total, rtol=1e-6)
end
...
@testitem "GlobalWarmingPotential structural verification" setup=[ClimateSetup] tags=[:climate] begin
    ...
    vars = unknowns(sys)
    var_names = [string(Symbolics.tosymbol(v, escape=false)) for v in vars]
    @test "GWP(t)" in var_names
    @test "AGWP_A(t)" in var_names
end
...
@testitem "GlobalWarmingPotential CO2 self-reference" setup=[ClimateSetup] tags=[:climate] begin
    ...
    for t_f in [20.0, 100.0, 500.0]
        prob = ODEProblem(csys, [], (0.0, 1.0), [csys.τ_A => 150.0, csys.a_A => 1.0, csys.t_f => t_f])
        sol = solve(prob, saveat=1.0)
        @test isapprox(sol[csys.GWP][1], 1.0, rtol=1e-6)
    end
end
...
@testitem "GWP_exponential function" setup=[ClimateSetup] tags=[:climate] begin
    ...
    @test isapprox(GWP_exponential(150.0, 1.0, 100.0), 1.0, rtol=1e-10)
    ...
    gwp_long = GWP_exponential(1e6, 1.0, 100.0)
