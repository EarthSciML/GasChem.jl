...
@testitem "OHProduction: Eq 6.3 OH Production Rate" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test P_OH_val > 1.0e12
    @test P_OH_val < 1.0e14
...
@testitem "OHProduction: Rate Constants at 298 K" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    @test param_dict[:k3_N2] ≈ 2.6e-11 * 1.0e-6  # m³/s
    @test param_dict[:k3_O2] ≈ 4.0e-11 * 1.0e-6  # m³/s
    @test param_dict[:k4] ≈ 2.2e-10 * 1.0e-6     # m³/s
    @test param_dict[:j_O3] ≈ 6.0e-5              # s⁻¹ (at surface, solar zenith 0°)
...
@testitem "OHProduction: Limiting Behavior - Humidity Variation" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    prob_low = NonlinearProblem(
        compiled,
        Dict(compiled.O3 => O3_val, compiled.H2O => 1.0e19, compiled.M => M_val);
        build_initializeprob = false
    )
    ...
    prob_high = NonlinearProblem(
        compiled,
        Dict(compiled.O3 => O3_val, compiled.H2O => 1.0e24, compiled.M => M_val);
        build_initializeprob = false
    )
...
@testitem "OHProduction: Eq 6.4 ε_OH vs Book Table (p. 207)" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    for (rh, eps_book) in book_values
        ...
        @test sol[compiled.ε_OH] ≈ eps_book rtol = 0.1
    end
end
...
@testitem "OHProduction: Qualitative - O1D Proportional to O3" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    # Solve with O3 = 1e18
    prob_a = NonlinearProblem(
        compiled,
        Dict(compiled.O3 => 1.0e18, compiled.H2O => H2O_val, compiled.M => M_val);
        build_initializeprob = false
    )
    sol_a = solve(prob_a)

    # Solve with O3 = 2e18
    prob_b = remake(prob_a, p = [compiled.O3 => 2.0e18])
    sol_b = solve(prob_b)

    # Doubling O3 should double O(1D) (linear relationship from Eq 6.1)
    @test sol_b[compiled.O1D] / sol_a[compiled.O1D] ≈ 2.0 rtol = 1.0e-6
end
...
@testitem "OHProduction: Qualitative - Positive Quantities" setup = [SP_CH6_Setup] tags = [:sp_ch6] begin
    ...
    for M_val in [1.0e24, 2.5e25, 1.0e26]
        for H2O_val in [1.0e21, 4.0e23, 1.0e24]
            for O3_val in [1.0e16, 1.0e18, 1.0e20]
                prob = NonlinearProblem(
                    compiled,
                    Dict(compiled.O3 => O3_val, compiled.H2O => H2O_val, compiled.M => M_val);
                    build_initializeprob = false
