...
struct GEOSChemGasPhase <: EarthSciMLODESystem
    ...
    function GEOSChemGasPhase(t)
        ...
        function arrsq(T, a0, b0, c0)
            sys = arrhenius(t, T, a0, b0, c0, unit=u"ppb^-2", name=Symbol(:arrhenius, i))
            i+=1
            push!(rate_systems, sys)
            return sys.k
        end
        ...
        rxn_sys = @reaction_network GEOSChemGasPhase begin
            ...
            @parameters(
                ...
                num_density = 2.7e19, [description = "Number density of air (The units should be molecules/cm^3 but the equationsn here treat it as unitless)."], k_mt1 = 0, [unit = u"ppb^-1", description = "Seasalt rate constant"],
