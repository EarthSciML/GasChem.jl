...
function direct_solar_beam_box(DTAUX, AMF2, P; threshold::Float64=76.0)
    ...
    if P < 1.0
        ...
        for k in 1:n_wave
            if AMF2[fine_index, fine_index] > 0.0
                tau = 0.5 * sum(DTAU[div(i + 1, 2), k] * AMF2[i, fine_index] for i in 1:ng)
...
function direct_solar_beam_box_singlewavelength(DTAUX, AMF2, P, k; threshold::Float64=76.0)
    ...
    if P < 1.0
        ...
        # compute the effective optical depth using the AMF2 weights via a comprehension.
        if AMF2[fine_index, fine_index] > 0.0
            tau = 0.5 * sum(DTAU[div(i + 1, 2), Int(k)] * AMF2[i, fine_index] for i in 1:ng)
            FTAU2 = tau < threshold ? exp(-tau) : 0.0
        end
    end
    return FTAU2
end
@register_symbolic direct_solar_beam_box_singlewavelength(DTAUX, AMF2, P, k)
