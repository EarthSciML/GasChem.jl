...
function Pollu(; name = :Pollu, rxn_sys = false)
    rx_sys = @reaction_network Pollu begin
        @ivs t [unit = u"s"]
        @parameters begin
            jNO2_O3P = 0.35 / 60.0, [unit = u"s^-1"]
            k2 = 0.266e2 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k3 = 0.123e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"] # updated to match Fontana pollu.f
            jH2COa = 0.86e-3 / 60.0, [unit = u"s^-1"]
            jH2COb = 0.82e-3 / 60.0, [unit = u"s^-1"]
            k6 = 0.15e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            jALD = 0.13e-3 / 60.0, [unit = u"s^-1"]
            k8 = 0.24e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k9 = 0.165e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k10 = 0.9e4 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            jPAN = 0.22e-1 / 60.0, [unit = u"s^-1"]
            k12 = 0.12e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k13 = 1.88 / 60.0, [unit = u"1/s"]
            k14 = 0.163e5 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k15 = 0.48e7 / 60.0, [unit = u"s^-1"]
            jO3_O1D = 0.35e-3 / 60.0, [unit = u"s^-1"]
            jO3_O3P = 0.175e-1 / 60.0, [unit = u"s^-1"]
            k18 = 0.1e9 / 60.0, [unit = u"s^-1"]
            k19 = 0.444e12 / 60.0, [unit = u"s^-1"]
            k20 = 0.124e4 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            jNO3_NO = 0.21e1 / 60.0, [unit = u"s^-1"]
            jNO3_NO2 = 0.578e1 / 60.0, [unit = u"s^-1"]
            k23 = 0.474e-1 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            k24 = 0.178e4 / (60.0 * 1.0e3), [unit = u"1/(ppb*s)"]
            jN2O5 = 0.312e1 / 60.0, [unit = u"s^-1"]
        end
        ...
        @species begin
            ...
            CO(t) = 100.0, [unit = u"ppb"]
            ALD(t) = 1.0e-11, [unit = u"ppb", description = "lumped non-formaldehyde aldehydes"]
            MEO2(t) = 4.0e-6, [unit = u"ppb", description = "CH3O2 (methyl peroxy radical)"]
            C2O3(t) = 0.0, [unit = u"ppb", description = "CH3C(O)O2 (acetyl peroxy radical)"]
            CO2(t) = 3.55e5, [unit = u"ppb"]
            PAN(t) = 1.0e-11, [unit = u"ppb", description = "CH3C(O)O2NO2 (peroxy nitrate)"]
            ...
            SO2(t) = 1.0e-11, [unit = u"ppb"]
            SO4(t) = 1.0e-11, [unit = u"ppb", description = "SO4 (sulfate)"]
            NO3(t) = 4.0e-6, [unit = u"ppb"]
            N2O5(t) = 4.0e-6, [unit = u"ppb"]
        end
    ...
    # See MTK docs and Catalyst.jl issue #311.
