...
The module provides three ModelingToolkit components and one pure function:

- `ClimateSensitivity()` - Climate sensitivity relations and feedback calculations
- `GHGForcing()` - Radiative forcing from well-mixed greenhouse gases
- `GlobalWarmingPotential()` - GWP calculation as a dynamic system
- `GWP_exponential()` - Direct GWP calculation for exponentially decaying species
...
The climate sensitivity equations relate radiative forcing perturbations to equilibrium temperature changes.
...
### Example: Climate Sensitivity Analysis
...
The parameters controlling the climate response:
...
The `GHGForcing` component computes radiative forcing contributions from the major well-mixed greenhouse gases using reference values from IPCC (2001).

### Reference Forcing Values (IPCC 2001)

From page 1039 of Seinfeld & Pandis (2006):
...
### GWP Time Horizon Dependence

GWP values depend strongly on the time horizon chosen. Short-lived species have higher GWPs at short time horizons, while long-lived species have relatively higher GWPs at longer horizons.
...
plot(t_horizons, gwp_short, label="τ=2 yr (short-lived)", lw=2)
plot!(t_horizons, gwp_medium, label="τ=12 yr (CH₄-like)", lw=2)
plot!(t_horizons, gwp_long, label="τ=114 yr (N₂O-like)", lw=2)
plot!(t_horizons, gwp_very_long, label="τ=3200 yr (SF₆-like)", lw=2)
xlabel!("Time Horizon (years)")
ylabel!("Global Warming Potential")
title!("GWP Dependence on Time Horizon")
```

This figure demonstrates Figure 23.15 from Seinfeld & Pandis (2006): short-lived species have high GWP at short time horizons but their relative importance diminishes over longer periods, while long-lived species show the opposite behavior.
